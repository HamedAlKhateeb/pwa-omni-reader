...
export default function Reader({ article, highlights, notes, settings, onClose, onSaveSettings, onUpdateArticle, onAddHighlight, onRemoveHighlight, onAddNote }: Props) {
  ...
  return <div className={`reader-view reader-theme-${settings.theme}`} id="reader-view" dir={settings.isRtl ? "rtl" : "ltr"}>
    ...
    {showSettings && <div className="reader-settings-panel"><button className="panel-close" onClick={() => setShowSettings(false)}><X size={17} /></button><div className="panel-heading"><Sparkles size={16} /> ضبط القراءة</div><label>حجم الخط <input type="range" min="14" max="32" value={settings.fontSize} onChange={(event) => onSaveSettings({ ...settings, fontSize: Number(event.target.value) })} /><b>{settings.fontSize}</b></label><label>تباعد السطور <input type="range" min="1.3" max="2.5" step="0.1" value={settings.lineHeight} onChange={(event) => onSaveSettings({ ...settings, lineHeight: Number(event.target.value) })} /><b>{settings.lineHeight.toFixed(1)}</b></label><label>تباعد الكلمات <input type="range" min="-1" max="8" step="0.5" value={settings.wordSpacing} onChange={(event) => onSaveSettings({ ...settings, wordSpacing: Number(event.target.value) })} /><b>{settings.wordSpacing}</b></label><label>عرض النص <input type="range" min="520" max="980" step="20" value={settings.width} onChange={(event) => onSaveSettings({ ...settings, width: Number(event.target.value) })} /></label><div className="choice-row">{(["serif", "sans", "mono"] as const).map((font) => <button key={font} className={settings.fontFamily === font ? "choice active" : "choice"} onClick={() => onSaveSettings({ ...settings, fontFamily: font })}>{font === "serif" ? "تقليدي" : font === "sans" ? "واضح" : "ثابت"}</button>)}</div><div className="choice-row">{(["right", "justify", "left"] as const).map((align) => <button key={align} className={settings.textAlign === align ? "choice active" : "choice"} onClick={() => onSaveSettings({ ...settings, textAlign: align })}>{align === "right" ? "يمين" : align === "justify" ? "ضبط" : "يسار"}</button>)}</div><div className="choice-row">{(["light", "cream", "sepia", "dark"] as const).map((theme) => <button key={theme} aria-label={theme} className={`theme-dot ${theme} ${settings.theme === theme ? "active" : ""}`} onClick={() => onSaveSettings({ ...settings, theme })} />)}</div><button className={settings.isRtl ? "direction-button active" : "direction-button"} onClick={() => onSaveSettings({ ...settings, isRtl: !settings.isRtl })}>اتجاه القراءة: {settings.isRtl ? "من اليمين" : "من اليسار"}</button></div>}
