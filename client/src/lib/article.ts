...
export function createArticle(url: string, title?: string, content = ""): Article { const now = Date.now(); const safeContent = cleanHtml(content, url); const words = wordCount(safeContent); let hostname = url; try { hostname = new URL(url).hostname.replace(/^www\./, ""); } catch { /* input is validated by caller */ } return { id: makeId("article"), url, title: title?.trim() || hostname || "مقال محفوظ", content: safeContent, excerpt: safeContent.replace(/<[^>]*>/g, " ").replace(/\s+/g, " ").trim().slice(0, 180), tags: [], savedAt: now, updatedAt: now, contentUpdatedAt: safeContent ? now : undefined, progress: 0, isRead: false, isArchived: false, isFavorite: false, readingTimeMinutes: words ? Math.max(1, Math.round(words / 220)) : 0, sourceStatus: safeContent ? "cached" : "link-only" }; }
